...
patiendId = 4 #in range(8,9): #range(1,12) #Use truncateDepth_initial=75, truncateDepth_final=50 for Patient=01 and 0,0 for Patient=04
...
######### Mean and Residual computation using Functional DVF while underlying PCA has been computed using interpol
assert True==args.prefilter and True==os.path.exists(args.pca_fn),\
    f"args.prefilter: {args.prefilter}, expected True; args.pca_fn {args.pca_fn} exists: {os.path.exists(args.pca_fn)} expected True."
print(f"args.prefilter: {args.prefilter}; args.pca_fn {args.pca_fn} exists: {os.path.exists(args.pca_fn)}.")
...
print('Setting prefilter flag of pca object to False.')
pca.prefilter=False
pca.mode='bilinear'

print('Recomputing mean volume using functional behaviour.')
log, residuals = pca.reconstruct_mean(vols, iterations=1000)

print(f'Reading  mean volume computed using Functional behaviour whereas PCA were computed using Interpol behaviour.')
mean, res, pos =  pca.mean_vol, pca.res, pca.pos

print(f'Preparing file names with Mixed behaviour.')
behaviourPrefixedConfigKey=behaviourPrefixedConfigKey.replace('Interpol', 'IntFuncMixed')
print(f'behaviourPrefixedConfigKey: {behaviourPrefixedConfigKey}')
name=name.replace('Interpol', 'IntFuncMixed')
print(f'name: {name}')
args.pca_fn=args.pca_fn.replace('Interpol', 'IntFuncMixed')
print(f'args.pca_fn: {args.pca_fn}')
args.reconstructedVol_fn=args.reconstructedVol_fn.replace('Interpol', 'IntFuncMixed')
print(f'args.reconstructedVol_fn: {args.reconstructedVol_fn}')

print(f'Writing mixed PCA')
pca.write(args.pca_fn)

print(f'Plotting logs and residuals from Mixed mean volume computation.')
if args.plot:
    fig, ax = plt.subplots()
